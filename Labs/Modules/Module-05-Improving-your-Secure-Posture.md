...
6. From the unhealthy tab, select both *mdclab-win* and *aslab-linux* virtual machines. Click **Fix**.
...
10. Ensure the VM extension is deployed on the relevant machines:
    ...
    - Repeat the process for **mdclab-linux** – you should expect to see a different name for the extension on Linux platform: `MDE.Linux`.
...
5. On the recommendation, expend **Affected resources**. You should see two unhealthy resources (mdclab-win and mdclab-linux) and not applicable resources.
6. From the **Unhealthy resources**, select **mdclab-win** resource. Here you can view all relevant recommendations for that resource.
...
2. Copy the name or your container registry, for example: *mdclabcrktfvrxcne4kki*
...
10. Expend the **Affected resources** section and notice the **Unhealthy registries** count which shows **1 container registry** (mdclab-xxx).
...
3. On the Basics tab, select **Azure subscription 1** and resource group **mdclab**.
...
### Continue with the next lab: [Module 6 - Workload Protections](../Modules/Module-06-Workload-Protections.md)
